...
public class CuesHttpHandler implements HttpHandler {
	...
	private final transient Logger logger = LoggersManager.getInstance().getLogger(CuesHttpHandler.class.getName());
	
	@Override
	public void handle(HttpExchange httpExchange) throws IOException {
		if(httpExchange.getRequestMethod().equalsIgnoreCase("GET")){
			try{
				HashMap<String, String> paramsMap = extractParams(httpExchange.getRequestURI().getQuery());
				...
				if(actionParam != null){
					if(actionParam.equals("unusedCueName")){
						...
						httpExchange.sendResponseHeaders(200, unusedCueName.length());
						httpExchange.getResponseBody().write(unusedCueName.getBytes());
			...
			try {
				...
				if(actionParam != null && cueNameParam != null){
					if(actionParam.equals("load")){
						artNetServerManager.setCurrentOutputDmxArray(cuesManager.getCue(cueNameParam).getDmxArray());
					}
					else if(actionParam.equals("store")){
						cuesManager.addCue(new Cue(cueNameParam, artNetServerManager.getCurrentOutputDmxArray()));
					}
					...
					logger.severe("actionParam="+actionParam+" cueName="+cueNameParam);
					throw new BadSyntaxException();
				}
